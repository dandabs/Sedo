...
exports.run = (client, message, args) => {
    ...
    let ref = db.collection('verification').doc(userid);

    let getDoc = ref.get()
  .then(doc => {
    if (doc.exists) {
        ref.delete();
        message.channel.send(":exclamation: I've deleted the verification request. You can now re-verify using !verify.").then(msg => {
          ...
          client.guilds.get(guild).members.get(userid).member.removeRoles(client.guilds.get(guild).members.get(userid).roles);
          client.guilds.get(testguild).members.get(userid).member.removeRoles(client.guilds.get(testguild).members.get(userid).roles);
